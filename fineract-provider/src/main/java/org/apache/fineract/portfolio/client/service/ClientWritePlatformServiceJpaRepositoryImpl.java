...
public class ClientWritePlatformServiceJpaRepositoryImpl implements ClientWritePlatformService {
    ...
    public CommandProcessingResult createClient(final JsonCommand command) {
        ...
        try {
            ...
            CodeValue eduLevel = null;
            final Long eduLevelId = command.longValueOfParameterNamed(ClientApiConstants.eduLevelIdParamName);
            if (eduLevelId != null) {
                eduLevel = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.EDU_LEVEL,
                        eduLevelId);
            }
            ...
            final Client newClient = Client.createNew(currentUser, clientOffice, clientParentGroup, staff, savingsProductId, gender,
                    clientType,eduLevel, clientClassification, legalFormValue, command);
    ...
    public CommandProcessingResult updateClient(final Long clientId, final JsonCommand command) {
        ...
        try {
            ...
            if (changes.containsKey(ClientApiConstants.eduLevelIdParamName)) {
                final Long newValue = command.longValueOfParameterNamed(ClientApiConstants.eduLevelIdParamName);
                CodeValue newCodeVal = null;
                if (newValue != null) {
                    newCodeVal = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.EDU_LEVEL,
                            newValue);
                }
                clientForUpdate.updateClientType(newCodeVal);
            }
