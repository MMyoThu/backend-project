...
public final class ClientDataValidator {
    ...
    public void validateForCreate(final String json) {
        ...
        if (this.fromApiJsonHelper.parameterExists(ClientApiConstants.monthlyIncomeParamName, element)) {
            final String mobileNo = this.fromApiJsonHelper.extractStringNamed(ClientApiConstants.monthlyIncomeParamName, element);
            baseDataValidator.reset().parameter(ClientApiConstants.mobileNoParamName).value(mobileNo).ignoreIfNull()
                    .notExceedingLengthOf(50);
        }
        ...
        if (this.fromApiJsonHelper.parameterExists(ClientApiConstants.eduLevelIdParamName, element)) {
            final Integer eduLevel = this.fromApiJsonHelper.extractIntegerSansLocaleNamed(ClientApiConstants.eduLevelIdParamName,
                    element);
            baseDataValidator.reset().parameter(ClientApiConstants.eduLevelIdParamName).value(eduLevel).integerGreaterThanZero();
        }
    ...
    public void validateForUpdate(final String json) {
        ...
        if (this.fromApiJsonHelper.parameterExists(ClientApiConstants.monthlyIncomeParamName, element)) {
            atLeastOneParameterPassedForUpdate = true;
            final String mobileNo = this.fromApiJsonHelper.extractStringNamed(ClientApiConstants.monthlyIncomeParamName, element);
            baseDataValidator.reset().parameter(ClientApiConstants.monthlyIncomeParamName).value(mobileNo).notExceedingLengthOf(50);
        }
